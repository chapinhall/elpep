...
show_acs1_cat_density_by_puma <- function(my_cat, my_subtitle) {
 acs1_elig %>% 
  ...
  geom_bar() +
  facet_grid(
    get(my_cat) ~ child_agegroup_combo,
    scales = "free_y"
  ) +
  labs(
...
acs1_puma_stats <- 
  ...
  summarize(
    .by = puma,
    pct_pov = sum(PWGTP[fam_incpov_ratio <= 1]) / sum(PWGTP)
  )
  ...
  acs5puma <- 
    ...
    summarize(
      .by = PUMA,
      rIncPov0to100_Age18to64 = sum(nIncPov0to100_Age18to64) / sum(wIncPov0to100_Age18to64)
    )
...
ggplot() +
  ...
  labs(
    title = "Comparing Distributions of Poverty Rates for Individuals Aged 18-64",
...
# /!\ GKFF -- need to supply multiple of these
...
# /!\ GKFF -- update specifications with new 01d measures
...
sae_controls_incpov_Age65plus <- 
  paste0(sae_controls_incpov_stems, "_Age65plus")
...
sae_controls_65plus_inc     <- c(sae_controls_incpov_Age65plus, sae_controls_ed)
sae_controls_65plus_incwork <- c(sae_controls_incpov_Age65plus, sae_controls_ed, sae_controls_workstatus)

stopifnot(all(c(sae_controls_inc, sae_controls_incwork) %in% colnames(acs5tract)))

```

```{r subset acs1 and acs5 data for the SAE}
acs1_elig[j = PUMA := str_pad(PUMA, width = 5, side = "left", pad = "0")]
...
specs <- 
  c(
    "sae_results_inc_019", 
    "sae_results_inc_2064",
    "sae_results_inc_65",
    
    "sae_results_incwork_019",
    "sae_results_incwork_2064",
    "sae_results_incwork_65"
  ) 
...
sae_controls_list <- 
  list(
    ...
    sae_controls_65plus_inc,
    sae_controls_019_incwork,
    sae_controls_2064_incwork,
    sae_controls_65plus_incwork
  )
