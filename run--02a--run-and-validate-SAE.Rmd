...
sae_controls_2644_inc       <- c(sae_controls_incpov_Age20to44, sae_controls_ed)
sae_controls_2644_incwork   <- c(sae_controls_incpov_Age20to44, sae_controls_ed, sae_controls_workstatus)
...
plot_sae_comp_by_tract(
  sae_results_2544_inc$sae_out,   
  base_cat = incpov_ref_value, 
  title = "Base Income Cat Composition by Tract -- Ages 25-44"
)
...
# plot_sae_comp_by_tract(
#   sae_results_2064_incwork$sae_out,   
#   base_cat = "WorkEligThresh_0%-75%", # /!\ Need to fix directly 
...
sae_results_2544_inc[["agg_comp_fh"]]$agg_comp_plot +
  labs(
    title = "PUMA-level SAE Estimates vs ACS1 Counts",
    subtitle = "Inc-to-Pov, Ages 25-44, FH Estimates",
    x = "Observed ACS 1-year Counts"
  ) 

sae_results_2544_inc[["agg_comp_model"]]$agg_comp_plot +
  labs(
    title = "PUMA-level SAE Estimates vs ACS1 Counts",
    subtitle = "Inc-to-Pov, Ages 25-44, Model Estimates",
    x = "Observed ACS 1-year Counts"
  ) 

# sae_results_2064_incwork[["agg_comp_fh"]]$agg_comp_plot +
#   labs(
#     title = "PUMA-level SAE Estimates vs ACS1 Counts",
...
#   ) 
# 
# sae_results_2064_incwork[["agg_comp_model"]]$agg_comp_plot +
#   labs(
#     title = "PUMA-level SAE Estimates vs ACS1 Counts",
...
cbind(
  sae_results_018_inc[["agg_comp_fh"]]$agg_comp_cor  %>% mutate(Age0to18  = r3(V1)) %>% select(-V1),
  sae_results_1925_inc[["agg_comp_fh"]]$agg_comp_cor %>% mutate(Age19to25 = r3(V1)) %>% select(Age19to25),
  sae_results_2644_inc[["agg_comp_fh"]]$agg_comp_cor %>% mutate(Age26to44 = r3(V1)) %>% select(Age26to44),
  sae_results_4564_inc[["agg_comp_fh"]]$agg_comp_cor %>% mutate(Age45to64 = r3(V1)) %>% select(Age45to64),
  sae_results_65_inc[["agg_comp_fh"]]$agg_comp_cor   %>% mutate(Age65plus = r3(V1)) %>% select(Age65plus)
  # sae_results_65plus_inc[["agg_comp_fh"]]$agg_comp_cor     %>% mutate(Age65plus = r3(V1)) %>% select(Age65plus),
  # sae_results_019_incwork[["agg_comp_fh"]]$agg_comp_cor    %>% mutate(Age0to19  = r3(V1)) %>% select(-V1),
  # sae_results_2064_incwork[["agg_comp_fh"]]$agg_comp_cor   %>% mutate(Age20to64 = r3(V1)) %>% select(Age20to64),
  # sae_results_65plus_incwork[["agg_comp_fh"]]$agg_comp_cor %>% mutate(Age65plus = r3(V1)) %>% select(Age65plus),
) %>% 
...
cbind(
  sae_results_018_inc[["agg_comp_model"]]$agg_comp_cor  %>% mutate(Age0to18  = r3(V1)) %>% select(-V1),
  sae_results_1925_inc[["agg_comp_model"]]$agg_comp_cor %>% mutate(Age19to25 = r3(V1)) %>% select(Age19to25),
  sae_results_2644_inc[["agg_comp_model"]]$agg_comp_cor %>% mutate(Age26to44 = r3(V1)) %>% select(Age26to44),
  sae_results_4564_inc[["agg_comp_model"]]$agg_comp_cor %>% mutate(Age45to64 = r3(V1)) %>% select(Age45to64),
  sae_results_65_inc[["agg_comp_model"]]$agg_comp_cor   %>% mutate(Age65plus = r3(V1)) %>% select(Age65plus)
  # sae_results_65plus_inc[["agg_comp_model"]]$agg_comp_cor     %>% mutate(Age65plus = r3(V1)) %>% select(Age65plus),
  # sae_results_019_incwork[["agg_comp_model"]]$agg_comp_cor    %>% mutate(Age0to19  = r3(V1)) %>% select(-V1),
  # sae_results_2064_incwork[["agg_comp_model"]]$agg_comp_cor   %>% mutate(Age20to64 = r3(V1)) %>% select(Age20to64),
  # sae_results_65plus_incwork[["agg_comp_model"]]$agg_comp_cor %>% mutate(Age65plus = r3(V1)) %>% select(Age65plus),
) %>% 
  kable(caption = "Correlation of PUMA-level Observed and SAE Values from the Fay-Herriot Method")
...
compare_sae_lt125fpl(
  sae_results_65plus_inc$sae_out,
  title = diagnostic_note,
...
if (developer_mode) {
  ...
  sae_results_test_inc <- make_acs5_equiv(sae_results_2064_inc$sae_out, "2064", only_hh_with_kids = FALSE, verbose = TRUE)
    ...
    .[between(AGEP, 0, 6),
      j = .(acs1_ccdf = 
              sum(PWGTP[fam_incpov_ratio < ccdf_inc_thresh_fpl/100 & 
                          bAdults_all_workThresh_sch == TRUE]) / sum(PWGTP),
            acs1_ccdf_inc  = sum(PWGTP[fam_incpov_ratio < ccdf_inc_thresh_fpl/100]) / sum(PWGTP),
            acs1_ccdf_work = sum(PWGTP[bAdults_all_workThresh_sch == TRUE])                       / sum(PWGTP)),
      by = PUMA] %>% 
...
if (FALSE) {
  ...
  sae_redis_incpov <- redis_sae(sae_results_2064_inc, "2064", only_hh_with_kids = FALSE)
    View(sae_redis_incpov$sae_out_decompress, "redis_inc")
    sae_redis_incpov$sae_out_decompress %>% select(GEOID, vc_value, matches("^share|^infl")) %>% View("check_sae_redis")
  # sae_redis_incwork <- redis_sae(sae_results_2064_incwork, "2064");
...
# Check on success
if (FALSE) {
  sub_cn(sae_results_2064_inc$sae_out_decompress, "_infl$")
  sae_results_2064_inc$sae_out_decompress %>% 
...
if (rerun_sae | !file.exists(sae_out_file)) {
  ...
  # Save the bundle of objects
  save(
    list = c(results, "specs", "incpov_ref_value", "sae_controls_list", "sae_aux_data_acs5"),
    file = sae_out_file
  )
