...
acs1_hoh <- 
  ...
  mutate(
    ...
    hoh_industry_vulnerable = 
      ifelse(hoh_industry == "NA", 
             "NA", 
             str_detect(hoh_industry, "educhealth|food")
               # Note: the second argument to str_detect() uses regular expressions. 
               # For reference, see e.g. https://regexone.com/
      ),
    hoh_race =
...
acs1_hh <-
  ...
  filter(
    cRel %in% c("Head", "Spouse") # subset to only householder and spouse records
  ) %>% 
  summarize(
    .by = c(YEAR, SERIALNO, fam_size),
    spouse_present = maxNA(cRel == "Spouse"),           # note: spouse_present is hh-level var
...
acs1_elig <- 
  ...
  mutate(
    ...
    fam_incpov_ratio_cat_hs_prog = bin_incpov_ratio(fam_incpov_ratio, pov_breaks = c(0, 1.00, prog_inc_thresh_fpl/100, 4.00)),
    ...
    cWorkElig      = ifelse(  all_work_sch == 1, "WorkElig",      "NotWorkElig"),
    cSpousePresent = ifelse(spouse_present == 1, "SpousePresent", "NoSpousePresent"),
    
    work_spouse_status    = paste_(cWorkElig, cSpousePresent),
    work_incpov_status185 = paste_(cWorkElig, fam_incpov_ratio_cat_to185),
    work_incpov_status100 = paste_(cWorkElig, fam_incpov_ratio_cat_by100),
    work_incpov_status75  = paste_(cWorkElig, fam_incpov_ratio_cat_by75),
    work_incpov_statushs  = paste_(cWorkElig, fam_incpov_ratio_cat_hs_prog),
...
acs1_elig <-
  acs1_elig %>% 
  as.data.table() %>% 
  .[j = paste0("prog_elig_incratio_", local_prog_incratio_cuts) :=
...
partition_vars <- 
  str_subset(cn(acs1_elig), "fam_incpov.+cat|work_spouse|work_incpov")
...
elig_stats_puma <- 
  ...
  mutate(
    prog_inc_elig = 
      fam_incpov_ratio_cat_hs_prog %in% 
        c("0%-100%", glue("100%-{local_prog_incratio_base}%")),
    prog_elig     = str_detect(work_incpov_statushs, "^WorkElig_(0%|100%)")) %>% 
  summarize(
    .by = PUMA,
    pct_le50_elig   = meanNA(fam_incpov_ratio_cat_mix == "0%-50%"),
    pct_le100_elig  = meanNA(fam_incpov_ratio_cat_by100 == "0%-100%"),
    pct_leProg_elig = meanNA(prog_inc_elig),
    pct_prog_elig   = meanNA(prog_elig),
    pct_work_elig_if_incLe225 = meanNA(all_work_sch[prog_inc_elig])
  )
```
...
ggplot(
  ...
  labs(
    title = glue("Comparison of PUMA-level Poverty and {local_prog_name_short} Eligibility"),
    subtitle = glue("Given the {local_prog_name_short} work requirement, the relative stability of this relationship may be surprising,\n",
                    "since labor force attachment may be expected to be lower, on average, in higher poverty\n",
                    "communities"),
    x = "Poverty Rate",
    y = glue("Rate of {local_prog_name_short} Eligibility")
  ) +
  theme_minimal()
```
...
ggplot(
  ...
  #coord_fixed() +
  labs(
    title = glue("PUMA-level Work vs Income-Eligibility for {local_prog_name_short} for Families of Children 0-5"),
    # /*\ Note: check that the pattern and thus interpretation holds
    subtitle = glue("The negative relationship leads to a stable overall {local_prog_name_short} eligiblity rate as PUMAs with lower\n",
...
elig_stats_puma_stack <-
  elig_stats_puma %>% 
  mutate(
    pct_prog_only     = pct_prog_elig,
    pct_le225_nonprog = pct_leProg_elig - pct_prog_elig
  ) %>% 
  pivot_longer(
    cols = -c(PUMA)
  )
...
elig_stats_puma_stack %>% 
  filter(
    name %like% "pct_prog_elig|pct_leProg_elig|pct_work_elig_if"
  ) %>%
  mutate(
    .by = PUMA,
    prog_val = value[name == "pct_prog_elig"]
  ) %>%  # pct_work_elig_if_le225
  arrange(prog_val) %>% 
  mutate(
    PUMA_order = match(PUMA, unique(PUMA))
  ) %>% 
  ggplot(
    aes(
      x = PUMA_order,
  ...
  labs(
    title = glue("PUMA-level Rates of {local_prog_name_short} Eligibility, With Work and Income Eligibility"),
    subtitle = glue("{local_prog_name_short} eligibility can often be composed of countervailing income and work eligibility"),
    x = glue("PUMAs sorted by % {local_prog_name_short} eligibility")
  ) + 
  theme_minimal()
```
...
(puma_agg <- 
   ...
   mutate(
     .by = PUMA,
     faminc_lt_thresh = fam_incpov_ratio <= local_prog_incratio_base/100,
   ...
   summarize(
     ...
     sample_n = n(),
     n = sum(PWGTP),
     n_ltProgThresh         =   comma(sum(PWGTP[])),
     pct_ltProgInc          = percent(sum(PWGTP[faminc_lt_thresh]) / sum(PWGTP), accuracy = 0.01),
     n_ltProgInc_workElig   =   comma(sum(PWGTP[faminc_lt_thresh & workelig])),
     pct_ltProgInc_workElig = percent(sum(PWGTP[faminc_lt_thresh & workelig]) / sum(PWGTP), accuracy = 0.01)
   ) %>% 
   arrange(PUMA))
...
acs1_elig <- 
  ...
  mutate(
    cAgegroup_combo = 
      case_when(
        cAgegroup %in% c("0to2", "3to5") ~ "0to5",
        TRUE ~ as.character(cAgegroup)
      ) %>% 
      factor(
        levels = 
          c(setdiff(levels(cAgegroup), 
                    c("0to2", "3to5")),
            "0to5")
      ) %>% 
      fct_relevel("0to5")) %>% 
  arrange(cAgegroup_combo, PUMA)
...
acs1_puma_stats <- 
  cbind(
    acs1_boot$data[j = .(cAgegroup_combo, PUMA)] %>% unique(),
