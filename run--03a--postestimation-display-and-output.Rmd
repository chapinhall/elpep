...
nowcast_parallel_incpov_le130 <- 
  nowcast_parallel_setup %>% 
  filter(outcome_var == "incpov_le130_post_adj",
         str_detect(sae_spec, "incpov_ctrls_100")) %>% 
...
pop_by_age_long <- 
  pop_by_age %>% 
  pivot_longer(
    cols = matches("age_.+_count")) %>% 
  mutate(
    age = str_replace(name, "_count", ""),
...
pop_by_age_long_recency <- 
  bind_rows(
    pop_by_age_long %>% 
      mutate(cps_base_recency = "allbase"),
    pop_by_age_long %>% 
      mutate(cps_base_recency = "most_recent"),
    pop_by_age_long_sy %>% 
...
nowcast_parallel_counts <- 
  ...
  merge(
    pop_by_age_long_recency,
    by = c("GEOID", "age", "pd", "cps_base_recency"),
    all.x = TRUE) %>% 
  data.table() %>%
  mutate(
