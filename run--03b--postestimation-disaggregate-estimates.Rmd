...
pop_age_mapped <- 
  ...
  mutate(
    age = 
      case_when(
        age_var %like% "Age(0to4|5to8|9to15|16to18)" ~ "Age0to18",
        age_var %like% "Age19to25"                   ~ "Age19to25",
        age_var %like% "Age(26to34|35to44)"          ~ "Age26to44",
        age_var %like% "Age(45to54|55to64)"          ~ "Age45to64"
        #age_var %like% "Age65plus"                   ~ "Age65plus"
      )
  ) %>% 
  # Remove un-mapped rows (which may represent other population cutpoints)
  filter(
    !is.na(age)
  ) %>% 
  mutate(
...
stopifnot(all(pop_age_mapped$age %in% nowcast_parallel$age))
