...
pop_by_age_long_0to5 <- 
  pop_by_age_long_recency %>% 
  select(
    GEOID, year, age, pd, cps_base_recency, pop_count, pop_count_se) %>% 
  filter(
    age %in% c("age_0", "age_1", "age_2", "age_3", "age_4", "age_5"))
  # Separate out the estimates for ages 0-5, which is the age group that we want
  # to disaggregate with other population breakdown information

nowcast_elig_counts_0to5 <- 
  nowcast_parallel_counts %>% 
  filter(age == "age_0to5") %>%
  select(-age, -pop_count, -pop_count_se) %>% 
  ...
  merge(
    pop_by_age_long_0to5,
    by = c("GEOID", "year", "pd", "cps_base_recency"),
    all.x = TRUE,
    allow.cartesian = TRUE) %>% 
  ...
  mutate(
    count    = pop_count*share,
    count_se = se_product(share, pop_count, share_se, pop_count_se))

# Add these by-single-age values to the other estimates we have so far
nowcast_elig_counts_added_ages <- 
  bind_rows(
    nowcast_elig_counts_0to5,
    nowcast_parallel_counts) %>% 
  select(
    outcome_var, GEOID, cps_base_recency, pd, year, age, 
    share, share_se, count, count_se, pop_count, pop_count_se) %>% 
    ...
    if (agg_level == "ZCTA") {
      ...
      geo_merge_data <- 
        ...
        transmute(GEOID         = GEOID, 
                  ...
                  geo_part      = pct_area)
    } else if (str_detect(my_output_tag, "IECAM")) {
      geo_merge_data <- 
        geo_crosswalk_b5 %>% 
        transmute(GEOID         = GEOID, 
                  b5            = b5, 
                  geo_part      = pct_b5)
    } else if (str_detect(my_output_tag, "CECIDS") && agg_level == "distr") {
      geo_merge_data <- 
        geo_crosswalk_distr %>% 
        transmute(GEOID         = GEOID, 
                  distr         = LONGNAME,
                  geo_part      = pct_area)
    } else if (str_detect(my_output_tag, "CECIDS") && agg_level == "network") {
      geo_merge_data <- 
...
nowcast_agg_st    <- agg_tracts_up(nowcast_elig_counts_final, "State")
...
if (str_detect(my_output_tag, "CECIDS")) {
  nowcast_agg_distr   <- agg_tracts_up(nowcast_elig_counts_final, "distr")
  nowcast_agg_network <- agg_tracts_up(nowcast_elig_counts_final, "network")
}
...
if (str_detect(my_output_tag, "CECIDS")) {
  my_write.csv(nowcast_agg_distr,   "by local school district")
  my_write.csv(nowcast_agg_network, "by CPS elementary network")
}
...
if (str_detect(my_output_tag, "CECIDS")) {
  to_save <- c(to_save, "nowcast_agg_distr")
  to_save <- c(to_save, "nowcast_agg_network")
}
