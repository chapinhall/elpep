...
#### Generate Excel Tables with Estimates at Each Geographic Aggregation
...
label_date_range <- 
  function(my_months) {
    ...
    date1 <- my_months_range[1]
    date2 <- my_months_range %>% .[length(.)]
...
if (FALSE) {
  label_date_range(most_recent_months)
  label_date_range(most_recent_sy_months)
}
```

```{r identify label for income measure used for thresholds}
prog_income_thresh_label <- 
...
write_agg_output <- 
  ...
  date_label <- 
    case_match(
      recency,
      "most_recent" ~ label_date_range(most_recent_months),
      "sy"          ~ label_date_range(most_recent_sy_months),
      "allbase"     ~ glue("Calendar Year {base_year + 1}"))
  ...
  for (lvl in agg_levels) {
    add_sheet <-
      ...
      filter(
        ...
        get(lvl[["sheet_name"]]) != "") %>% 
          # remove rows with blank values for the geography
      ...
      mutate(
        Age = 
          age %>% 
          str_replace("Age(\\d+)to(\\d+)", "Age \\1-\\2") %>% 
          str_replace("Age(\\d+)plus", "Age \\1\\+") %>% 
          str_replace("^n", "") %>% 
          sort_by_char_nums(),
        
        Outcome = 
          case_when(
            str_detect(outcome_var, "^bProg") ~ 
              str_replace(
                outcome_var, 
                "bProg.+_(\\d+)",
                glue("Program eligible: Working, Income <\\1% {prog_income_thresh_label}")),
            
            # /!\ In cases of other thresholds being of interest, could
            # replace these "_le(\\d+)_" assignments with a more robust
            # one using regular expressions
            str_detect(outcome_var, "_le(\\d+)")  ~ 
              str_replace(
                outcome_var,
                ".+_le(\\d+).*",
                "Below \\1% FPL")
      ...
      select(-age, -count_se, -share_se, -pop_count_se) %>% 
      
      pivot_longer(
        cols = matches("(count|share)(_|$)")
      ) %>% 
      mutate(
        stat = 
          case_when(
            name == "pop_count" ~ "Overall #",
            name %like% "count" ~ "#",
            name %like% "share" ~ "%"
          ),
        calc = 
          case_when(
            name %like% "_lb" ~ ", lower bound",
...
outcomes_out_regex <- glue("bProg_elig_|bIncpov_le({output_fpl_cuts_regex})")

lAgg_for_output <- 
  list(
    list(
      agg_table = nowcast_agg_tract,
      agg_field = "GEOID",
      sheet_name = "Census Tract"
    ),
    list(
      agg_table = nowcast_agg_zip,
      agg_field = "ZCTA",
      sheet_name = "Zip Code"
    ),
    list(
      agg_table = nowcast_agg_cty,
      agg_field = "County",
      sheet_name = "County"
    ),
    list(
      agg_table = nowcast_agg_sd,
      agg_field = "school",
      sheet_name = "School Districts"
    ),
    list(
      agg_table = nowcast_agg_st,
      agg_field = "State",
      sheet_name = "State"
    )
  )
...
write_agg_output(
  outcomes_regex = outcomes_out_regex,
  #age_regex      = "age(_3$|_4$|_infant$|_toddler$)", # "_0to2|_3to5|_0to5|_6to12"
  agg_levels     = lAgg_for_output,
  recency        = "most_recent", # /*\ Other options can be
    # "allbase" for calendar year of base_year + 1
  out_label      = "Medicaid and SNAP" # local_prog_name_short
)
...
```{r examine the dispersion of program eligibility for potentially multiple income thresholds}

nowcast_agg_zip %>% 
  filter(
    outcome_var %like% "^bProg",
    pd == "nowcast"
  ) %>% 
  ggplot() +
  aes(
  ...
  scale_color_discrete(
    name = "Income Eligibility Threshold"
  ) +
  labs(
    x = "% Prog Eligibility"
  ) +
...
if (exists("custom_income_thresh")) {
  ...
  get_zcta_order <- function(z) plot_data %>% filter(ZCTA == z) %>% pull(share) %>% min()
  
  my_plot <-
    plot_data %>% 
    ggplot() +
    aes(
      x = reorder(ZCTA, order),
      y = share,
      color = inc_thresh
    ) +
    geom_point(
    ...
    labs(
      x = "ZCTAs, in order of lowest income threshold"
    ) +
    theme(
      legend.position = "bottom",
...
<!-- date_range <- label_date_range(most_recent_months) -->
...
<!--     estimates %>%  -->
<!--     filter( -->
<!--       outcome_var %like% outcome, -->
<!--       cps_base_recency == case_match(plot_stat, -->
<!--                                      "nowcast" ~ "most_recent", -->
<!--                                      "diff"    ~ "allbase") -->
<!--     ) %>%  -->
<!--     select(-count, -count_se, -pop_count, -pop_count_se, -share_se, -cps_base_recency) -->
...
<!--       "viridis" -->
<!--     ) # this is roughly yellow-green-purple -->
...
<!--     ) + -->
<!--     labs( -->
<!--       title    = ifelse(!is.null(map_title), map_title, str_wrap(measure_label)), -->
...
<!-- # /*\ This code is not (yet) applicable to runs requested for OK, but is left -->
<!-- # here for adaptation if desired -->

<!-- if (FALSE) { -->
<!--   for (out_var in c("ccdf", "le100", "le200")) { -->

<!--     if (out_var == "ccdf") { -->
<!--       out_var_desc <- glue("{local_ccdf_name_short} - {local_ccdf_incratio_base}% {prog_income_thresh_label} ({date_range})") -->
...
<!--     static_map_level_diff( -->
<!--       estimates = nowcast_agg_zip %>% filter(age == age_agg_main), -->
<!--       geo_level = "ZCTA", -->
...
<!--     static_map_level_diff( -->
<!--       estimates = nowcast_agg_cty %>% filter(age == age_agg_main), -->
<!--       geo_level = "County", -->
<!--       outcome = out_var_name, -->
<!--       field_desc = out_var_desc, -->
<!--       save_map = TRUE,  -->
...
<!--     static_map_level_diff( -->
<!--       estimates = nowcast_agg_sd %>% filter(age == age_agg_main), -->
<!--       geo_level = "school", -->
<!--       outcome = out_var_name, -->
<!--       field_desc = out_var_desc, -->
<!--       save_map = TRUE,  -->
...
<!--     if (exists("my_aux_geo")) { -->
<!--       static_map_level_diff( -->
<!--         estimates = nowcast_agg_aux %>% filter(age == age_agg_main), -->
<!--         geo_level = "aux", -->
...
<!--                    "{inc_thresh}% {prog_income_thresh_label}, {age_desc} ", -->
<!--                    "({date_range})"), -->
...
<!-- ```{r remove portions of shapes that are over lake michigan, eval = str_detect(my_output_tag, "CECIDS|IECAM")} -->

<!-- #install.packages(c("rnaturalearth", "rnaturalearthdata")) -->
<!-- library(rnaturalearth) -->
<!-- library(rnaturalearthdata) -->

<!-- target_crs <- st_crs(tractShp) -->
...
<!-- lake_michigan <- lakes %>% filter(name == "Lake Michigan") -->

<!-- # Function to subtract the lake from tracts that touch Chicago (or from all tracts) --- -->
<!-- subtract_lake <- function(my_shp) { -->

<!--   my_shp <- st_transform(my_shp, target_crs) -->

<!--   # This subsets to only Chicago intersections, temporarily -->
<!--   # idx_touch_chi <- lengths(st_intersects(my_shp, chi_map)) > 0 -->
<!--   # my_shp_chi <- my_shp[idx_touch_chi, ] -->

<!--   tracts_no_lake <- st_difference(my_shp, st_union(lake_michigan)) -->
<!--   tracts_no_lake <- tracts_no_lake[!st_is_empty(tracts_no_lake), ] -->
<!--     # Drop any empty geometries (purely water tracts become empty after subtraction) -->

<!--   #rbind(my_shp[!idx_touch_chi, ], tracts_no_lake)   -->
<!--   return(tracts_no_lake) -->
<!-- } -->

<!-- tractShp_nolake <- subtract_lake(tractShp) -->
<!-- pumaShp_nolake  <- subtract_lake(pumaShp) -->
<!-- auxShp_nolake   <- subtract_lake(auxShp) -->
<!-- ``` -->

<!-- ```{r Function to build the bare bones of the 'story' maps} -->
...
<!--     shpEsts %>%  -->
<!--     ggplot() + -->
<!--     #annotation_map_tile(type = "cartolight", zoomin = -1, alpha = 0.7) + -->
<!--     geom_sf( -->
...
<!--       alpha = 1) +  -->
<!--     coord_sf( -->
<!--       xlim = with(bbox, c(xmin, xmax)), -->
<!--       ylim = with(bbox, c(ymin, ymax))) + -->
<!--     theme_void() -->

<!--   ggsave( -->
<!--     filename = glue("{output_path}Story Map -- {save_tag}.png"), -->
<!--     plot = story_plot) -->
<!--   # width = X, -->
<!--   # height = X, -->
<!--   # units = "in -->
...
<!-- ```{r generate progression of maps to explain story of estimation} -->

<!-- ### These are defaults set for telling the story of estimation -->
<!-- # /!\ In the future, these could be generalized as inputs -->
<!-- story_ages <- "Pre-K 4" -->
<!-- story_outcome <- "ccdf_elig_incratio_225" -->
<!-- story_ccdf_cut <- "225" -->
<!-- story_recency <- "most_recent" -->
<!-- story_bbox <- list(xmin = -87.672438, xmax = -87.561779, ymin = 41.757867, ymax = 41.821629) -->
<!--   # This can be identified from web tools like: https://tools.mofei.life/bbox -->


<!-- ### Plot final, now-cast estimates -->
<!-- story_ests_final <-  -->
<!--   nowcast_agg_tract %>%  -->
<!--   filter( -->
<!--     age == story_ages, -->
<!--     outcome_var == glue("ccdf_elig_incratio_{story_ccdf_cut}"), -->
<!--     pd == "nowcast", -->
<!--     cps_base_recency == story_recency) -->
<!-- shp_final <-  -->
<!--   tractShp_nolake %>%  -->
<!--   merge( -->
<!--     story_ests_final, -->
<!--     by = "GEOID") -->
<!-- val_limits <- range(story_ests_final$share, na.rm = T) -->
...
<!--   val_limits,  -->
<!--   story_bbox,  -->
<!--   glue("final ELPEP CCDF, {story_ages}")) -->

<!-- # If the output is specific to CECIDS or IECAM, create a second plot highlighting Hyde Park -->
<!-- if (str_detect(my_output_tag, "CECIDS|IECAM")) { -->
<!--   story_map( -->
<!--     shp_final,  -->
<!--     fill_var = "share", -->
<!--     val_limits,  -->
<!--     story_bbox,  -->
<!--     glue("final ELPEP CCDF, {story_ages}, HP highlight"),  -->
<!--     add_hp = TRUE)   -->
<!-- } -->

<!-- ### Base SAE estimates -->

<!-- ### ACS1 as basis, but highly aggregate -->
<!-- load(file = glue("{output_path}acs1_data_{my_output_tag}.Rda")) -->
<!-- story_ests_acs1 <-  -->
<!--   acs1_child %>%  -->
<!--   filter( -->
<!--     AGEP %in% c(0, 5)) %>%  -->
...
<!--     .by = PUMA, -->
<!--     share =  -->
<!--       sum(PWGTP[get(glue("ccdf_elig_incratio_{story_ccdf_cut}")) == 1]) / -->
<!--       sum(PWGTP)) -->
<!-- shp_acs1 <-  -->
<!--   pumaShp_nolake %>%  -->
<!--   merge( -->
...
<!--   val_limits,  -->
<!--   story_bbox,  -->
<!--   glue("ACS1 CCDF, {story_ages}")) -->


<!-- ### ACS5 as predictor: wrong measure, but captures spatial heterogeneity -->
<!-- load(file = glue("{output_path}acs5_data_{my_output_tag}.Rda")) -->
<!-- story_ests_acs5 <-  -->
<!--   tractShp_nolake %>%  -->
<!--   merge( -->
<!--     acs5tract %>%  -->
<!--       select(GEOID, incpov_lt6_r0to100_est, incpov_lt6_r100to199_est) %>%  -->
<!--       mutate(share = incpov_lt6_r0to100_est + incpov_lt6_r100to199_est), -->
<!--     by = "GEOID") -->

<!-- story_map( -->
<!--   story_ests_acs5, -->
<!--   fill_var = "share", -->
<!--   val_limits = range(story_ests_acs5$share),  -->
<!--   story_bbox, -->
<!--   glue("ACS5 200pct FPL, {story_ages}"),  -->
<!--   color_opt = "plasma") -->
