      ...
      mutate(
        Outcome = 
          case_when(str_detect(outcome_var, "^ccdf") ~ 
                    ...
                    str_detect(outcome_var, "_le130_") ~ "Below 130% FPL",
...
if (str_detect(my_output_tag, "CECIDS")) {
  lAgg_for_output[[length(lAgg_for_output)+1]] <- 
    list(agg_table = nowcast_agg_distr,
         agg_field = "distr",
         sheet_name = "Local School District")
  lAgg_for_output[[length(lAgg_for_output)+1]] <- 
    list(agg_table = nowcast_agg_network,
         agg_field = "network",
         sheet_name = "CPS Elementary Network")
}
...
write_agg_output(
  ...
  recency        = ifelse(str_detect(my_output_tag, "CECIDS"), "sy", "most_recent"),  
