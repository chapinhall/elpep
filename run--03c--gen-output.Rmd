...
write_agg_output <- function(outcomes_regex, age_regex = ".+", agg_levels, recency = "most_recent", out_label) {
  ...
  for (lvl in agg_levels) {
    add_sheet <-
      copy(lvl[["agg_table"]]) %>%
      setnames(lvl[["agg_field"]], "agg_field_label") %>% 
      mutate(agg_field_label = sort_by_char_nums(agg_field_label)) %>% 
      arrange(agg_field_label) %>% 
      setnames("agg_field_label", lvl[["sheet_name"]]) %>% 
      filter(pd == "nowcast",
...
output_ccdf_cuts_regex <- 
  local_ccdf_incratio_cuts %>% 
  str_split(pattern = ",") %>% 
  unlist() %>% 
  str_trim() %>% 
  paste(collapse = "|")

outcomes_out_regex <- glue("ccdf_elig_incratio_({output_ccdf_cuts_regex})|incpov_le({output_fpl_cuts_regex})")

# Order output to be roughly geographically largest to smallest (tracts last)
lAgg_for_output <- 
  list(list(agg_table = nowcast_agg_all,
            agg_field = "all",
            sheet_name = my_state),
...
lAgg_for_output[[length(lAgg_for_output)+1]] <- 
  list(agg_table = nowcast_agg_tract,
       agg_field = "GEOID",
