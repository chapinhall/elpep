...
cps_transition_pov_unadj <- 
  calc_cps_transitions(base_data = cps_child_base[YEAR == base_year & METRO %in% 2:4],
...
cps_child_base[j = fhoh_empl := factor(hoh_empl, 
                                       ...
                                       labels = c("HOH Not Emp, Base",
                                                  "HOH Emp, Base"))]
transition_ratios_empl_base <-
  calc_cps_transitions(base_data = cps_child_base[YEAR == base_year & MONTH == 1], 
                       stratum_vars = NULL, 
                       vulnerability_vars = c("fam_incpov_ratio_cat_by100_adj", 
                                              "fhoh_empl"),
...
visualize_cps_transitions(trans_calcs = transition_ratios_empl_base, 
                          vul_vars = c("fam_incpov_ratio_cat_by100_adj",
                                       "fhoh_empl"),
                          vul1_label = "Income-to-Poverty at Baseline",
...
transition_ratios_empl_base_metro <-
  calc_cps_transitions(base_data = cps_child_base[YEAR == base_year & MONTH == 1 & METRO %in% 1:4] %>% 
...
visualize_cps_transitions(trans_calcs = transition_ratios_empl_base_metro, 
                          vul_vars = c("fam_incpov_ratio_cat_by100_adj",
                                       "METRO"),
                          vul1_label = "Income-to-Poverty at Baseline",
...
trans_ratios_base       <- trans_ratio_calc_byinput(cps_child_base[YEAR == base_year],                  "All CPS")
trans_ratios_metro      <- trans_ratio_calc_byinput(cps_child_base[YEAR == base_year & METRO %in% 2:4], "Metro")
trans_ratios_citycenter <- trans_ratio_calc_byinput(cps_child_base[YEAR == base_year & METRO == 2],     "City Center")
```
...
trans_ratios_base_01  <- trans_ratio_calc_byinput(cps_child_base[METRO %in% 2:4 & YEAR == base_year & MONTH == 1], "Jan Cohort")
trans_ratios_base     <- trans_ratio_calc_byinput(cps_child_base[METRO %in% 2:4 & YEAR == base_year],              "All Cohorts")
...
comp_ratios_cohort <- 
  bind_rows(trans_ratios_base,
            trans_ratios_base_01
            # trans_ratios_2020_02,
            # trans_ratios_2020_win
            ) %>% 
  mutate(outcome_label = label_outcome(outcome_var),
         cohorts = factor(subset_label, 
                          levels = c("All Cohorts", "Jan Cohort"))) # , "Feb 2020 Cohort", "Winter 19/20 Cohort"
...
While the SAE method is limited in how much detail it can estimate for `r `base_year` with validity, this method uses other measures by assuming that ACS 5-year measures are applicable to `r base_year` For some measures like adult educational attainment and race/ethnicity, we believe that this assumption is defensible.
