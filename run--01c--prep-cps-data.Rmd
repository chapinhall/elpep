...
cps_sub <- 
  ...
  dplyr::select(-COUNTY, -OCC, -CLASSWKR) # dropping variables not currently used.
...
cps_family <- 
  ...
  summarize(
    ...
    single_filer_flag = RELATE == 101 & MARST %in% 2:6 & n_qual_kids == 0, 
      # this is head who is unmarried (single, separated, divorced or widowed) with no qualifying kids
    joint_filer_flag  = RELATE == 101 & MARST == 1,                        
      # assumed MFJ if married, not considering the case of married, living separately and filing MFS
    hoh_filer_flag    = RELATE == 101 & MARST %in% 2:6 & n_qual_kids > 0
      # this is head who is unmarried (single, separated, divorced or widowed) with qualifying kids %>% 
  ) %>% 
  mutate(
    has_schage_kid_base = 
      factor(
        has_schage_kid_base_flag,
        levels = 0:1, 
        labels = c("No Sch-Age Kid, Base", "Has Sch-Age Kid, Base")),
    has_young_kids_base = 
      factor(
        1*(n_kids_le5 > 0),
        levels = 0:1,
        labels = c("No Young Kids, Base", "1+ Young Kids, Base")
      )
    ) %>% 
  mutate(
    .by = c(YEAR, MONTH, YEAR_MONTH, CPSID),
    single_filer_flag = max(single_filer_flag),
    joint_filer_flag  = max( joint_filer_flag),
    hoh_filer_flag    = max(   hoh_filer_flag),
    
    filing_status = 
      case_when(
        single_filer_flag == 1 ~ "single", 
...
```{r}
# /!\ Turn this into a "stopifnot()" calculation
...
cps_pov <-
  ...
  mutate(
    ...
    # /!\ Check to ensure that independent measures of income interest are built here
    fam_incpov_ratio_le.5       = 1*(fam_incpov_ratio     <= 0.5),
    fam_incpov_ratio_le1        = 1*(fam_incpov_ratio     <= 1.0),
    fam_incpov_ratio_le1.30     = 1*(fam_incpov_ratio     <= 1.30),
...
if (exists("custom_income_thresh")) {
  ...
  if (!maxNA(cps_pov$fam_size) %in% custom_income_thresh$fam_size) {
    ...
    # /!\ Need to make this a `stopifnot()` condition
...
cps_post <-
  cps_elig[is_post == TRUE & RELATE == 101 & months_since_base == 15,
           j = c("CPSID",
                 ...
                 "fam_incpov_ratio_le1.30",
                 "fam_incpov_ratio_le1.30_adj",
  ...
  rename(incpov_le50_post      = fam_incpov_ratio_le.5,
         ...
         incpov_le130_post     = fam_incpov_ratio_le1.30,
         incpov_le130_post_adj = fam_incpov_ratio_le1.30_adj,
  ...
  unique() %>% 
    # The unique() is used to collapse household/HOH fields (which is all that is
    # preserved) to the household (designated by CPSID) rather than individual record
  filter(CPSID %in% unique(cps_child_base$CPSID))
    # Only preserve records for households that correspond to a child record
```
...
if (2019 %in% cps_hoh_pre_post$YEAR_b) {
