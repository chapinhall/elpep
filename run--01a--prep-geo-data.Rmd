...
```{r add local school districts for CECIDS}

if (str_detect(my_output_tag, "CECIDS")) {
  # Read in auxiliary data shape
  distrShp <- st_read(glue("{input_path}ERSB_20_Sub_District_Map_FA1_SB_15.shp"))
  my_distr_geo_field <- "LONGNAME"
  
  # Give a generic name to the aux geography's label field
  distrShp$distr_geo_label <- distrShp %>% pull(get(my_distr_geo_field))

  # Reproject if necessary
  if (st_crs(distrShp) != st_crs(tractShp)) {
    distrShp <- 
      st_transform(distrShp,
                   crs = st_crs(tractShp))
  }
  
  # Attempt to identify intersections
  tract_distr_intersection <- 
    st_intersection(
      tractShp,
      distrShp)
  # This returns all geographies of tract sliced by distr, i.e. 
    # representing potential duplicates
  sf_use_s2(FALSE)
    # This above step is recommended to avoid findings of degenerate geometries due
    # to duplicate vertices: https://github.com/r-spatial/sf/issues/1762
  
  tract_distr_intersection$area <- as.numeric(st_area(tract_distr_intersection))
  ...
  if (FALSE) {
    tract_distr_intersection %>% 
      group_by(GEOID) %>% 
      filter(n() > 1) %>% 
      select(GEOID, distr_geo_label, area) %>% 
      arrange(GEOID) %>% 
      View()
  }
  ...
  # Merge the original tract file into this intersections file to be able to
  # reference the total area of each tract
  # Note: we do this instead of relying on sum(area) because tract_distr_intersection
  # represents only polygons that intersect with the distr geography, which can
  # fully remove portions of tracts that are outside of the distr boundaries, leading
  # to erroneous filtering and proration of tract-level statistics
  tract_area <- 
    tractShp %>% 
    mutate(tract_area = as.numeric(st_area(tractShp)))
  
  tract_distr_intersection_pcts <- 
    tract_distr_intersection %>% 
    # Merge tract_area contents as new data.frame (which drops the geometry field,
    # and avoids errors)
    merge(with(tract_area,
  ...
  # Examine the histogram of prorations
  if (FALSE) {
    # Look at whether it's all 100%s or basically 100%s
    tract_distr_intersection_pcts %>% 
      ggplot() +
      geom_histogram(aes(x = pct_area))
    
    # Look at the size of tracts that might have splits
    # Are areas that are split higher percentages (e.g. 50-50 instead of 99-1) 
...
```{r add CPS school networks for CEDCIDS}
if (str_detect(my_output_tag, "CECIDS")) {
  # Read in auxiliary data shape
  networkShp <- st_read(glue("{input_path}geo_export_d32291d4-ca2f-4110-8a91-07eee30bd746.shp"))
  my_network_geo_field <- "planningzo"
  
  # Give a generic name to the aux geography's label field
  networkShp$network_geo_label <- networkShp %>% pull(get(my_network_geo_field))
  # Reproject if necessary
  if (st_crs(networkShp) != st_crs(tractShp)) {
    networkShp <- 
      st_transform(networkShp,
                   crs = st_crs(tractShp))
  }
  
  # Attempt to identify intersections
  tract_network_intersection <- 
    st_intersection(
      tractShp,
      networkShp)
  # This returns all geographies of tract sliced by network, i.e. 
    # representing potential duplicates
  sf_use_s2(FALSE)
    # This above step is recommended to avoid findings of degenerate geometries due
    # to duplicate vertices: https://github.com/r-spatial/sf/issues/1762
  
  tract_network_intersection$area <- as.numeric(st_area(tract_network_intersection))
  ...
  if (FALSE) {
    tract_network_intersection %>% 
      group_by(GEOID) %>% 
      filter(n() > 1) %>% 
      select(GEOID, network_geo_label, area) %>% 
      arrange(GEOID) %>% 
      View()
  }
  ...
  # Merge the original tract file into this intersections file to be able to
  # reference the total area of each tract
  # Note: we do this instead of relying on sum(area) because tract_network_intersection
  # represents only polygons that intersect with the network geography, which can
  # fully remove portions of tracts that are outside of the network boundaries, leading
  # to erroneous filtering and proration of tract-level statistics
  tract_area <- 
    tractShp %>% 
    mutate(tract_area = as.numeric(st_area(tractShp)))
  
  tract_network_intersection_pcts <- 
    tract_network_intersection %>% 
    # Merge tract_area contents as new data.frame (which drops the geometry field,
    # and avoids errors)
    merge(with(tract_area,
